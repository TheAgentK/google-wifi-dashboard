...
export const store = createStore({
  ...
  actions: {
    ...
    async setGroup(context) {
      ...
      context.commit(
        "setPrivateSsid",
        group.groupSettings.wlanSettings.privateSsid
      );
      context.commit("setWifiDevices", group.accessPoints);
    },
    async setDevices(context) {
      const devices = (
        await getGoogleWifiApi().getGroupDevices(context.state.group.id)
      ).stations;
      context.commit("setDevices", devices);
    },
    async getSpeedTestResults(context) {
      const results = (
        await getGoogleWifiApi().getGroupSpeedTestResults(
    ...
    async setRealtimeMetrics(context) {
      const metrics = await getGoogleWifiApi().getGroupRealtimeMetrics(
        context.state.group.id
      );
    ...
    async setInsightCards(context) {
      const cards = (
        await getGoogleWifiApi().getGroupInsightCards(context.state.group.id)
      ).insightsCards;
